	...
	{	for(num=1;num<=10;num++)
		{	switch(m[from][num])
			{	case 9: stab[from][at[from][num]][num]=1;attack[from][at[from][num]][num]=2;pause[from]++;damage[from][at[from][num]][num]=1; break;
				...
				case 19: stab[from][at[from][num]][num]=5;attack[from][at[from][num]][num]=5;pause[from]=pause[from]+6;damage[from][at[from][num]][num]=1; break;
				case 20: stab[from][at[from][num]][num]=6;attack[from][at[from][num]][num]=5;pause[from]=pause[from]+10;damage[from][at[from][num]][num]=2; break;
			}
		}
	}
...
{	for(from=1;from<=people;from++)
	{	switch(m[from][1])
		{	case 1: add[from][from]=1; break;
			case 7: add[from][from]=2; break;
		}
	}
}
...
{	for(from=1;from<=people;from++)
	{	switch(m[from][1])
		{	case 6: ca[from]=1; break;
			case 8: ca[from]=2; break;
			case 21: ca[from]=3; break;
			case 22: ca[from]=4; break;
		}
	}
}
...
{		for(from=1;from<=people;from++)
	{	switch(ca[from])
		{	case 1:bullet[from]--;break;
			case 2:pause[from]++;break;
			case 3:pause[from]=pause[from]+3;break;
			case 4:pause[from]=pause[from]+3;break;
		}
		int x;
		x=0;//暂时用于判断的变量
		for(num=1;num<=people;num++)
		{	if(stab[from][at[from][num]][num]==0 && (ca[from]==1 || ca[from]==0));
			else x++;
		}
		if(x==0)
		pause[from]=0;
...
{		printf("\n");
	...
	{	for(num=1;num<=people;num++)
		...
		{	case 1:printf("player%d charge the bullet\n",from); break;
			...
			case 19:printf("player%d ghost-chop at%d\n",from,to); break;
			case 20:printf("player%d nock-chop at%d\n",from,to); break;
			case 21:printf("player%d disarm\n",from); break;
			case 22:printf("player%d specular reflection\n",from); break;
			case 23:printf("player%d giant shield\n",from); break;
		}
		}
	}
}
...
{	printf("最多9个人\n 子弹bullet1 \n 给枪shoot2:消耗一颗子弹 \n 双枪doubleshoot3:消耗两颗子弹 \n 挡block4:格挡单枪和刺 \n 双挡doubleblock5:格挡双枪和刺 \n 反弹rebound6:反弹单枪的伤害，格挡刺 \n 拔剑drawasword7:剑可无限次使用，但消耗完可用的剑后不能再连续使用 \n 亮剑brandish8:反弹对方刺的攻击 \n 刺stab9:需一把剑 \n 砍chop10:需两把剑 \n 重挡heavy block11:格挡砍\n 激光枪laser gun12:消耗2子弹，造成2伤害\n 激光炮laser cannon13:消耗（对方HP-1）子弹，一击致命\n 火箭炮RPG14:消耗3子弹，造成2伤害\n 双火箭炮double RPG15:消耗6子弹，造成3伤害\n 核弹nuclear bomb16:消耗5子弹，对目标造成3伤害，对其他人造成1伤害\n 光剑lightsaber-stab17:需3剑，造成2伤害的普通刺\n 光刀lightsaber-chop18:需5剑，造成2伤害的普通砍\n 鬼斩ghost-chop19:需6剑\n 箭尾斩nock-chop20:需10剑，造成2伤害的鬼斩\n 缴械disarm21:防御除了鬼斩外的剑的伤害并使攻击者下回合无法用剑\n 镜面反射specular reflection22:反弹光类伤害\n 巨盾giant shield23:挡住鬼斩与箭尾斩\n攻击力比较：激光炮<刺=光剑<砍=光刀<单枪=双枪=激光枪<鬼斩=箭尾斩<RPG=双RPG<核弹\n核弹只能单独使用；其他攻击可对不同玩家同时进行\n");
	...
	{	turn++;
		...
		//其他类型 
		if(m[1][1]==1 || m[1][1]==4 || m[1][1]==5 || m[1][1]==6 || m[1][1]==7 || m[1][1]==8 || m[1][1]==11 || m[1][1]==21 || m[1][1]==22 || m[1][1]==23 )at[1][1]=1;
		else//攻击类型 
		{
		printf("目标？");
		scanf("%d",&at[1][1]);
		...
		{		int multiattack=0;
				...
				{		for(from=0;from<=9;from++)
					...
					do{arbitraryrange(m[j][1],choice);//multiattack这个变量在这里作用不是scanf，而是保证通过x随机判定的可以进入multiattack模式的不会因进入第二个循环而无法multiattack 
					}while((m[j][1]==0 || m[j][1]==1 || m[j][1]==4 || m[j][1]==5 || m[j][1]==6 || m[j][1]==7 || m[j][1]==8 || m[j][1]==11 || m[j][1]==21 || m[j][1]==22 || m[j][1]==23)&& multiattack==2);//第一次multiattack没什么用？但是若未通过benefit就有用了 
				if(HP[j]<=0)m[j][1]=0;
				if(m[j][1]==0 || m[j][1]==1 || m[j][1]==4 || m[j][1]==5 || m[j][1]==6 || m[j][1]==7 || m[j][1]==8 || m[j][1]==11 || m[j][1]==21 || m[j][1]==22 || m[j][1]==23 )at[j][1]=j;
				else//决定multiatttack 
				{
					...
					{	//add one more attack
						...
						}while(m[j][num]==0 || m[j][num]==1 || m[j][num]==4 || m[j][num]==5 || m[j][num]==6 || m[j][num]==7 || m[j][num]==8 || m[j][num]==11 || m[j][num]==21 || m[j][num]==22 || m[j][num]==23);//不打死人，不打自己
