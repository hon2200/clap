...
//			case 22: ca[from]=3; break;
//			case 23: ca[from]=4; break;
//		}
//	}
//}
//
...
//		doubleattackcheck();
//		//其他类型 
//		if(m[1][1]==1 || m[1][1]==4 || m[1][1]==5 || m[1][1]==6 || m[1][1]==7 || m[1][1]==8 || m[1][1]==11 || m[1][1]==22 || m[1][1]==23 || m[1][1]==24 )at[1][1]=1;
//		else//攻击类型 
//		{
//		printf("目标？");
//		scanf("%d",&at[1][1]);
...
//					}while((m[j][1]==0 || m[j][1]==1 || m[j][1]==4 || m[j][1]==5 || m[j][1]==6 || m[j][1]==7 || m[j][1]==8 || m[j][1]==11 || m[j][1]==22 || m[j][1]==23 || m[j][1]==24)&& multiattack==2);//第一次multiattack没什么用？但是若未通过benefit就有用了 
//				if(HP[j]<=0)m[j][1]=0;
//				//其他类型 
//				if(m[j][1]==0 || m[j][1]==1 || m[j][1]==4 || m[j][1]==5 || m[j][1]==6 || m[j][1]==7 || m[j][1]==8 || m[j][1]==11 || m[j][1]==22 || m[j][1]==23 || m[j][1]==24 )at[j][1]=j;
//				//攻击类型 
//				else//决定multiatttack
//				{
...
//						}while(m[j][num]==0 || m[j][num]==1 || m[j][num]==4 || m[j][num]==5 || m[j][num]==6 || m[j][num]==7 || m[j][num]==8 || m[j][num]==11 || m[j][num]==22 || m[j][num]==23 || m[j][num]==24);//不打死人，不打自己
