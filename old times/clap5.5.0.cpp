...
//int comeon[10];
//int provoke[10][10];//provoke少了to一个参量，理论上应该有，但因为provoke不会因过来而重叠，所以这里偷懒了 
//int somebodycomeon;
...
//void act();
//void printact();
//void clsatbegin();//瞬时值清零
//void clseveryturn();//输入值清零 
//void clsbeforesimulation();//状态值转存
//void clsaftersimulation();//状态值转存
//void clsafterall();//瞬时值加状态值清零
//void clsafteract();//用xx_转存瞬时值 
//void rule__();
//void introduction();
...
//				case 13: shoot[from][at[from][num]][num]=4;attack[from][at[from][num]][num]=1;bullet[from]=bullet[from]-max(HP[at[from][num]]-1,2);damage[from][at[from][num]][num]=HP[at[from][num]]; break;
...
//	{	
//		for(num=1;num<=10;num++)
//		{	if(provoke_[from][num]==1)
//			{	
//				for(i=1;i<=10;i++)
//				{
//					if(attack[at_[from][num]][from][i]!=0)
//					x++;
//				}
//				if(x==0)
//				HP[at_[from][num]]--;
...
//		ds2[from][to]=0;
//		add[from][to]=0;
//		provoke[from][to]=0;//双瞬时值 
//		for(num=1;num<=people;num++)
//		{
//		attack[from][to][num]=0;
//		damage[from][to][num]=0;
//		stab[from][to][num]=0;
//		shoot[from][to][num]=0;//三瞬时值 
//		}
//		}
//		}
//}
...
//			provoke_[from][to]=provoke[from][to];
//			m_[from][to]=m[from][to];
//			at_[from][to]=at[from][to];
//		}
//	}
//	for(from=1;from<=people;from++)
...
//		}
//	}
//	while(doubleattack[1]>0 || (comeon_[1]==1 && m[1][1]==25) || (turn==1 && m[1][1]==25))
//	{	comeoncheck();
//		doubleattackcheck();
//	}
//} 
...
//	{	
//		for(num=1;num<=10;num++)
//		{	printf("player%d provoke player%d\t%d in%d num%d\n",from,at[from][num],provoke[from][num],provoke_[from][num],num);
//		}
//	}
//}
